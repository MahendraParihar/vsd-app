...
import {IJobList, ITableListFilter, LabelKey} from "@vsd-common/lib";
...
export class JobComponent implements OnInit, AfterViewInit {
  ...
  dataSource!: TableDataDatasource<IJobList>;
  totalCount = 0;
  ...
  constructor(
    ...
    this.title = this.labelService.getLabel(LabelKey.SIDE_MENU_JOB);
  ...
  add() {
    this.navigationService.navigateTo(NavigationPathEnum.JOB_MANAGE);
  }

  edit(obj: IJobList) {
    this.navigationService.navigateToById(NavigationPathEnum.JOB_MANAGE, obj.jobId);
  }

  async changeStatus(status: boolean, index: number, obj: IJobList) {
    await this.service.changeStatus(obj.jobId, !obj.active);
  ...
  async loadDataSet(): Promise<void> {
    ...
    this.payload.limit = this.paginator
      ...
      : MASTER_PAGE_SIZE;
    await this.dataSource.loadData(JobApiUrl.JOB, this.payload);
  }
}

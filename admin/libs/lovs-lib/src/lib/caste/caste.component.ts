...
import {ICasteList, ITableListFilter, LabelKey} from '@vsd-common/lib';
...
export class CasteComponent implements OnInit, AfterViewInit {
  labelKeys = LabelKey;
  title!: string;
  displayedColumns = [
    'seqNo',
  ...
  dataSource!: TableDataDatasource<ICasteList>;
  totalCount = 0;
  ...
  constructor(
    ...
    this.title = this.labelService.getLabel(LabelKey.SIDE_MENU_CASTE);
  ...
  add() {
    this.navigationService.navigateTo(NavigationPathEnum.CASTE_MANAGE);
  }

  edit(obj: ICasteList) {
    this.navigationService.navigateToById(NavigationPathEnum.CASTE_MANAGE, obj.casteId);
  }

  async changeStatus(status: boolean, index: number, obj: ICasteList) {
    await this.service.changeStatus(obj.casteId, !obj.active);
  ...
  async loadDataSet(): Promise<void> {
    ...
    this.payload.limit = this.paginator
      ...
      : MASTER_PAGE_SIZE;
    await this.dataSource.loadData(LovApiUrl.CASTE, this.payload);
  }
}

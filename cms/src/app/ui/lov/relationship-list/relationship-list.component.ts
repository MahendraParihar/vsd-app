...
import {RelationshipDatasource} from "../datasource/relationship.datasource";
...
export class RelationshipListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new RelationshipDatasource(this.httpService, this.snackBarService);
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.RELATION_SHIP_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.RELATION_SHIP_MANAGE, id);
  }

  onDeleteClick(item: RelationshipModel, index: number) {
  ...
  async updateStatusTask(item: RelationshipModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.RELATION_SHIP_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

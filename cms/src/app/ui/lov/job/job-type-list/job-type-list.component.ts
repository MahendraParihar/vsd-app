...
import {JobTypeDatasource} from "../../datasource/job-type.datasource";
...
export class JobTypeListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new JobTypeDatasource(this.httpService, this.snackBarService);
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.JOB_TYPE_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.JOB_TYPE_MANAGE, id);
  }

  onDeleteClick(item: JobTypeModel, index: number) {
  ...
  async updateStatusTask(item: JobTypeModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.JOB_TYPE_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

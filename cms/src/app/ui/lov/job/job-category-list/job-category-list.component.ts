...
import {JobCategoryDatasource} from "../../datasource/job-category.datasource";
...
export class JobCategoryListComponent implements OnInit, AfterViewInit, OnDestroy {

  displayedColumns = ["seqNo", 'title', 'status', "action"];
  dataSource: JobCategoryDatasource;
  totalCount = 0;
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new JobCategoryDatasource(this.httpService, this.snackBarService);
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.JOB_CATEGORY_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.JOB_CATEGORY_MANAGE, id);
  }

  onDeleteClick(item: JobCategoryModel, index: number) {
  ...
  async updateStatusTask(item: JobCategoryModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.postRequest(ApiUrlEnum.JOB_CATEGORY_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

...
import {StateDatasource} from "../../datasource/state.datasource";
...
export class StateListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new StateDatasource(this.httpService, this.snackBarService);
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.STATE_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.STATE_MANAGE, id);
  }

  onDeleteClick(item: StateModel, index: number) {
  ...
  async updateStatusTask(item: StateModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.STATE_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

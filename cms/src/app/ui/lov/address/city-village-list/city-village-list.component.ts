...
import {CityVillageDatasource} from "../../datasource/city-village.datasource";
import {CityVillageModel} from "../../../../models/city-village.model";
import {AlertTypeEnum} from "../../../../enum/alert-type-enum";

@Component({
  selector: 'app-address-list',
  templateUrl: './city-village-list.component.html',
  styleUrls: ['./city-village-list.component.scss']
})
export class CityVillageListComponent implements OnInit, AfterViewInit, OnDestroy {

  displayedColumns = ["seqNo", 'title', 'district','state','country','status', "action"];
  dataSource: CityVillageDatasource;
  totalCount = 0;
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new CityVillageDatasource(this.httpService, this.snackBarService);
  ...
  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.CITY_VILLAGE_MANAGE, id);
  }

  onDeleteClick(item: CityVillageModel, index: number) {
  ...
  async updateStatusTask(item: CityVillageModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.CITY_VILLAGE_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

...
export class CountryListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  displayedColumns = ["seqNo", 'title', 'code', 'status', "action"];
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new CountryDatasource(this.httpService, this.snackBarService);
  ...
  onDeleteClick(item: CountryModel, index: number) {
  ...
  async updateStatusTask(item: CountryModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.COUNTRY_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

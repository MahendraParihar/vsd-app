...
import {ServiceDatasource} from "../../datasource/service.datasource";
...
export class ServiceListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new ServiceDatasource(this.httpService, this.snackBarService);
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.SERVICE_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.SERVICE_MANAGE, id);
  }

  onDeleteClick(item: ServiceModel, index: number) {
  ...
  async updateStatusTask(item: ServiceModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.SERVICE_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

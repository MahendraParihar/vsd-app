...
import {BusinessDatasource} from "../../datasource/business.datasource";
...
export class BusinessListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new BusinessDatasource(this.httpService, this.snackBarService);
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.BUSINESS_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.BUSINESS_MANAGE, id);
  }

  onDeleteClick(item: BusinessModel, index: number) {
  ...
  async updateStatusTask(item: BusinessModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.BUSINESS_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

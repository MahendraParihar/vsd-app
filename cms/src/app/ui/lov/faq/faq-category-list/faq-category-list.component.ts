...
import {FaqCategoryDatasource} from "../../datasource/faq-category.datasource";
...
export class FaqCategoryListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new FaqCategoryDatasource(this.httpService, this.snackBarService);
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.FAQ_CATEGORY_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.FAQ_CATEGORY_MANAGE, id);
  }

  onDeleteClick(item: FaqCategoryModel, index: number) {
  ...
  async updateStatusTask(item: FaqCategoryModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.postRequest(ApiUrlEnum.FAQ_CATEGORY_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

...
import {AddictionDatasource} from "../../datasource/addiction.datasource";
import {AlertTypeEnum} from "../../../../enum/alert-type-enum";

@Component({
  selector: 'app-addiction-list',
  templateUrl: './addiction-list.component.html',
  styleUrls: ['./addiction-list.component.scss']
})
export class AddictionListComponent implements OnInit, AfterViewInit, OnDestroy {

  displayedColumns = ["seqNo", 'title', 'status', "action"];
  dataSource: AddictionDatasource;
  totalCount = 0;
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new AddictionDatasource(this.httpService, this.snackBarService);
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.ADDICTION_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.ADDICTION_MANAGE, id);
  }

  onDeleteClick(item: AdditionModel, index: number) {
  ...
  async updateStatusTask(item: AdditionModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.ADDICTION_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

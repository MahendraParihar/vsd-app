...
export class PostListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new PostDatasource(this.httpService, this.snackBarService);
  ...
  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.POST_MANAGE, id);
  }

  onDeleteClick(item: PostModel, index: number) {
  ...
  async updateStatusTask(item: PostModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.POST_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

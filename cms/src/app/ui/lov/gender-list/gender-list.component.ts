...
import {GenderDatasource} from "../datasource/gender.datasource";
...
export class GenderListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new GenderDatasource(this.httpService, this.snackBarService);
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.GENDER_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.GENDER_MANAGE, id);
  }

  onDeleteClick(item: GenderModel, index: number) {
  ...
  async updateStatusTask(item: GenderModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.GENDER_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

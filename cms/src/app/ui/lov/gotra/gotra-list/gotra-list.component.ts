...
import {GotraDatasource} from "../../datasource/gotra.datasource";
...
@Component({
  ...
  styleUrls: ['./gotra-list.component.scss']
})
export class GotraListComponent implements OnInit, AfterViewInit, OnDestroy {

  displayedColumns = ["seqNo", 'title', 'status', "action"];
  dataSource: GotraDatasource;
  totalCount = 0;
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new GotraDatasource(this.httpService, this.snackBarService);
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.GOTRA_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.GOTRA_MANAGE, id);
  }

  onDeleteClick(item: GotraModel, index: number) {
  ...
  async updateStatusTask(item: GotraModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.GOTRA_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

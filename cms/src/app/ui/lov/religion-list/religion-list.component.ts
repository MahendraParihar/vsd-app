...
import {ReligionDatasource} from "../datasource/religion.datasource";
...
export class ReligionListComponent implements OnInit, AfterViewInit, OnDestroy {

  displayedColumns = ["seqNo", 'title', 'status', "action"];
  dataSource: ReligionDatasource;
  totalCount = 0;
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new ReligionDatasource(this.httpService, this.snackBarService);
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.RELIGION_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.RELIGION_MANAGE, id);
  }

  onDeleteClick(item: ReligionModel, index: number) {
  ...
  async updateStatusTask(item: ReligionModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.RELIGION_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

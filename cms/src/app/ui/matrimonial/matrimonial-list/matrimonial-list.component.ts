...
import {MatrimonialDatasource} from "../matrimonial.datasource";
...
@Component({
  ...
  styleUrls: ['./matrimonial-list.component.scss']
})
export class MatrimonialListComponent implements  OnInit, AfterViewInit, OnDestroy {

  displayedColumns = ["seqNo", 'title', 'status', "action"];
  dataSource: MatrimonialDatasource;
  totalCount = 0;
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new MatrimonialDatasource(this.httpService, this.snackBarService);
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.MATRIMONIAL_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.MATRIMONIAL_MANAGE, id);
  }

  onDeleteClick(item: MatrimonialModel, index: number) {
  ...
  async updateStatusTask(item: MatrimonialModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.MATRIMONIAL_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

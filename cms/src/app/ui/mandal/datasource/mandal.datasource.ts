...
export class MandalDatasource implements DataSource<MandalModel> {

  private dataSubject = new BehaviorSubject<MandalModel[]>([]);
  ...
  connect(collectionViewer: CollectionViewer): Observable<MandalModel[]> {
  ...
  async loadData(payload: {}): Promise<boolean> {
    ...
    const apiResponse = await this.httpService.getRequest(ApiUrlEnum.MANDAL_LIST, payload, true);
    if (!apiResponse) {
      return false;
    }
    switch (apiResponse.code) {
      case ServerResponseEnum.SUCCESS:
        ...
        const tempList: MandalModel[] = [];
        for (let s of apiResponse.data.list) {
          tempList.push(MandalModel.fromJson(s));
        }

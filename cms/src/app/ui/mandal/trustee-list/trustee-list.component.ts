...
import {TrusteeDatasource} from "../datasource/trustee.datasource";
...
@Component({
  ...
  styleUrls: ['./trustee-list.component.scss']
})
export class TrusteeListComponent implements OnInit, AfterViewInit, OnDestroy {

  displayedColumns = ["seqNo", 'title', 'status', "action"];
  dataSource: TrusteeDatasource;
  totalCount = 0;
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new TrusteeDatasource(this.httpService, this.snackBarService);
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.TRUSTEE_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.TRUSTEE_MANAGE, id);
  }

  onDeleteClick(item: TrusteeModel, index: number) {
  ...
  async updateStatusTask(item: TrusteeModel, index: number): Promise<void> {
    const payload = {
      active: !item.active
    };

    const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.TRUSTEE_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

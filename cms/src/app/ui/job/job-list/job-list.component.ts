...
@ViewChild(MatPaginator) paginator: MatPaginator;
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new JobDatasource(this.httpService, this.snackBarService);
  ...
  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.JOB_MANAGE, id);
  }

  onDeleteClick(item: JobModel, index: number) {
  ...
  async updateStatusTask(item: JobModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      active: !item.active
    };

  const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.JOB_UPDATE_STATUS, payload, true);
  if (res) {
    switch (res.code) {

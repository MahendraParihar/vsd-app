...
import {AppUserDatasource} from "../app-user.datasource";
...
export class AppUserListComponent implements  OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private fb: UntypedFormBuilder,
    ...
    this.dataSource = new AppUserDatasource(this.httpService, this.snackBarService);
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.APP_USER_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.APP_USER_MANAGE, id);
  }

  onDeleteClick(item: AppUserModel, index: number) {
  ...
  async updateStatusTask(item: AppUserModel, index: number): Promise<void> {
    const payload = {
      id: item.id,
      flag: !item.active
    };

    const res: ResponseDataModel = await this.httpService.putRequest(ApiUrlEnum.APP_USER_UPDATE_STATUS, payload, true);
    if (res) {
      switch (res.code) {

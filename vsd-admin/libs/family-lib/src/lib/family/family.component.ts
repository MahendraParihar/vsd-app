...
export class FamilyComponent implements OnInit, AfterViewInit {
  ...
  add() {
    this.addEvent.emit(NavigationPathEnum.FAMILY_MANAGE);
  }

  edit(obj: IFamilyList) {
    this.editEvent.emit({ path: NavigationPathEnum.FAMILY_MANAGE, id: obj.familyId });
  }

  details(obj: IFamilyList) {
    this.editEvent.emit({path: NavigationPathEnum.FAMILY, id: obj.familyId});
  }

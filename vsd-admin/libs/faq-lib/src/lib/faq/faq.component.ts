...
import { IFaqList, ITableListFilter, LabelKey } from '@vsd-common/lib';
import { FaqService } from '../faq.service';
import { FaqApiUrl } from '../api-url';

@Component({
  selector: 'faq-lib-faq-list',
...
export class FaqComponent implements OnInit, AfterViewInit {
  ...
  dataSource!: TableDataDatasource<IFaqList>;
  totalCount = 0;
  ...
  add() {
    this.addEvent.emit(NavigationPathEnum.FAQ_MANAGE);
  }
  ...
  async changeStatus(index: number, obj: IFaqList) {
    await this.service.changeStatus(obj.faqId, !obj.active);
  ...
  async loadDataSet(): Promise<void> {
    ...
    await this.dataSource.loadData(FaqApiUrl.FAQ, this.payload);
  }
}

...
@Component({
  selector: 'lovs-lib-address-type',
  standalone: false,
...
export class AddressTypeComponent implements OnInit, AfterViewInit {
  ...
  add() {
    this.addEvent.emit(NavigationPathEnum.ADDRESS_TYPE_MANAGE);
  }

  edit(obj: IAddressTypeList) {
    this.editEvent.emit({ path: NavigationPathEnum.ADDRESS_TYPE_MANAGE, id: obj.addressTypeId });
  }

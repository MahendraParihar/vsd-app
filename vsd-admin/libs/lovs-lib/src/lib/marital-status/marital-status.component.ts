...
@Component({
  selector: 'lovs-lib-marital-status',
  standalone: false,
...
export class MaritalStatusComponent implements OnInit, AfterViewInit {
  ...
  add() {
    this.addEvent.emit(NavigationPathEnum.MARITAL_STATUS_MANAGE);
  }

  edit(obj: IMaritalStatusList) {
    this.editEvent.emit({path: NavigationPathEnum.MARITAL_STATUS_MANAGE, id:obj.maritalStatusId});
  }

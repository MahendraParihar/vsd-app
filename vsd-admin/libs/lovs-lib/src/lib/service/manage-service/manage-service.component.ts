...
import { FileTypeEnum, IManageService, InputLength, LabelKey, MediaForEnum } from '@vsd-common/lib';
import { ServiceService } from '../service.service';

@Component({
  selector: 'lovs-lib-manage-service',
...
export class ManageServiceComponent implements OnInit {
  ...
  lovModel!: IManageService;

  formGroup: FormGroup = new FormGroup({
    service: new FormControl(null, [Validators.required, Validators.maxLength(InputLength.CHAR_50)]),
  });
  ...
  constructor( private service: ServiceService,
    ...
    this.pageTitle = this.labelService.getLabel(this.id ? this.labelKeys.EDIT_SERVICE : this.labelKeys.ADD_SERVICE);

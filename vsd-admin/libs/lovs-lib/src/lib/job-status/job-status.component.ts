...
import {IJobStatusList, ITableListFilter, LabelKey} from '@vsd-common/lib';
import {FormControl} from "@angular/forms";
import {Title} from "@angular/platform-browser";
import {CountryService} from "../country/country.service";

@Component({
  selector: 'lovs-lib-job-status',
  standalone: false,
...
export class JobStatusComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.title = this.labelService.getLabel(LabelKey.SIDE_MENU_JOB_STATUS);
  ...
  add() {
    this.addEvent.emit(NavigationPathEnum.JOB_STATUS_MANAGE);
  }

  edit(obj: IJobStatusList) {
    this.editEvent.emit({path: NavigationPathEnum.JOB_STATUS_MANAGE, id: obj.jobStatusId});
  }

  async changeStatus(status: boolean, index: number, obj: IJobStatusList) {
    await this.service.changeStatus(obj.jobStatusId, !obj.active);

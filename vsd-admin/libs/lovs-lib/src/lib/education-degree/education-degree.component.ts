...
@Component({
  selector: 'lovs-lib-education-degree',
  standalone: false,
...
export class EducationDegreeComponent implements OnInit, AfterViewInit {
  ...
  add() {
    this.addEvent.emit(NavigationPathEnum.EDUCATION_DEGREE_MANAGE);
  }

  edit(obj: IEducationDegreeList) {
    this.editEvent.emit({path: NavigationPathEnum.EDUCATION_DEGREE_MANAGE, id: obj.educationDegreeId});
  }

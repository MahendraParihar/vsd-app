...
import { IBusinessList, ITableListFilter, LabelKey } from '@vsd-common/lib';
import { FormControl } from '@angular/forms';
import { Title } from '@angular/platform-browser';
import { BusinessService } from './business.service';

@Component({
  selector: 'lovs-lib-business',
  standalone: false,
...
export class BusinessComponent implements OnInit, AfterViewInit {
  ...
  dataSource!: TableDataDatasource<IBusinessList>;
  totalCount = 0;
  ...
  constructor(
    ...
    this.title = this.labelService.getLabel(LabelKey.SIDE_MENU_BUSINESS);
  ...
  add() {
    this.addEvent.emit(NavigationPathEnum.BUSINESS_MANAGE);
  }

  edit(obj: IBusinessList) {
    this.editEvent.emit({ path: NavigationPathEnum.BUSINESS_MANAGE, id: obj.businessId });
  }

  async changeStatus(status: boolean, index: number, obj: IBusinessList) {
    await this.service.changeStatus(obj.businessId, !obj.active);
  ...
  async loadDataSet(): Promise<void> {
    ...
    await this.dataSource.loadData(LovApiUrl.BUSINESS, this.payload);
  }
}

...
@Component({
  selector: 'lovs-lib-matrimonial-status',
...
export class MatrimonialStatusComponent implements OnInit, AfterViewInit {
  ...
  add() {
    this.addEvent.emit(NavigationPathEnum.MATRIMONIAL_STATUS_MANAGE);
  }

  edit(obj: IMatrimonialStatusList) {
    this.editEvent.emit({ path: NavigationPathEnum.MATRIMONIAL_STATUS_MANAGE, id: obj.matrimonialStatusId });
  }

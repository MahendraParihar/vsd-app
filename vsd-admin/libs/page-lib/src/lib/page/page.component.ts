...
export class PageComponent implements OnInit, AfterViewInit {
  labelKeys = LabelKey;
  title!: string;
  displayedColumns = ['seqNo', 'title', 'active', 'createdByUser', 'createdAt', 'updatedByUser', 'updatedAt', 'action'];
  ...
  add() {
    this.addEvent.emit(NavigationPathEnum.PAGE_MANAGE);
  }

  edit(obj: ILegalPageList) {
    this.editEvent.emit({path: NavigationPathEnum.PAGE_MANAGE, id:obj.legalPageId});
  }
  ...
  async loadDataSet(): Promise<void> {
    ...
    await this.dataSource.loadData(PageApiUrl.PAGES, this.payload);
  }
}

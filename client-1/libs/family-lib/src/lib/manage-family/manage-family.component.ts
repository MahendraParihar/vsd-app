...
export class ManageFamilyComponent implements OnInit, OnDestroy {
  ...
  formGroup: FormGroup = new FormGroup({
    firstName: new FormControl(null, [
      Validators.required,
      Validators.maxLength(InputLength.CHAR_50),
    ]),
    ...
    emailId: new FormControl(null, [
      Validators.required,
      Validators.email,
      Validators.maxLength(InputLength.CHAR_100),
    ]),
  });
  ...
  constructor(
    ...
    this.pageTitle = this.labelService.getLabel(
      this.id ? this.labelKeys.EDIT_FAMILY : this.labelKeys.ADD_FAMILY
    );
    ...
    try {
      ...
      this.snackBarService.showSuccess(
        this.labelService.getLabel(
          this.id
            ? this.labelKeys.SUCCESS_DATA_UPDATED
            : this.labelKeys.SUCCESS_DATA_ADDED
        )
      );
      this.onCancel();
    } catch (e) {
      this.snackBarService.showError(
        this.labelService.getLabel(this.labelKeys.ERROR_SOMETHING_WENT_WRONG)
      );
    }
  }
}

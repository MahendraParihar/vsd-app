...
import { FileTypeEnum, IManageCaste, InputLength, LabelKey, MediaForEnum } from '@vsd-common/lib';
import { CasteService } from '../caste.service';

@Component({
  selector: 'lib-manage-caste',
  standalone: false,
  templateUrl: './manage-caste.component.html',
  styleUrl: './manage-caste.component.scss',
})
export class ManageCasteComponent implements OnInit {
  ...
  lovModel!: IManageCaste;

  formGroup: FormGroup = new FormGroup({
    caste: new FormControl(null, [Validators.required, Validators.maxLength(InputLength.CHAR_50)]),
  });

  constructor(private activatedRoute: ActivatedRoute, private service: CasteService,
    ...
    this.pageTitle = this.labelService.getLabel(this.id ? this.labelKeys.EDIT_CASTE : this.labelKeys.ADD_CASTE);
